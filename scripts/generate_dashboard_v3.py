...
# Status mappings
STATUS_COMPLETED = {'ConcluÃ­do', 'Cancelado'}
...
def get_status_category(status: str) -> str:
    ...
    s_lower = s.lower().replace('Ã­','i').replace('Ãº','u')
...
def is_cloud_migration(summary: str) -> bool:
    ...
    keywords = ['Migration Module', 'Autolac Cloud', 'MigraÃ§Ã£o']
...
def detect_porte(summary: str) -> Tuple[str, int, int]:
    ...
    elif 'medium' in summary_lower or 'mÃ©dio' in summary_lower:
...
def generate_backlog_data(technicians_dict: Dict, epics: List[Dict], today: str) -> Dict:
    ...
    for epic in novo_epics:
        ...
        if status_cat == 'done' or status.lower().replace('Ã­','i') in ('concluido', 'cancelado'):
            continue
    ...
    for epic in epics:
        ...
        if not impl or impl in EXCLUDE_ASSIGNEES:
            ...
            if 'interlac' in summary.lower():
                ...
                tipo = 'NF'
            elif 'cloud' in summary.lower() or 'migraÃ§Ã£o' in summary.lower():
                tipo = 'Cloud'
    ...
    for tech_name in IMPLEMENTERS:
        ...
        risco = 'ALTO' if ocupacao > 100 else 'MÃDIO' if ocupacao > 50 else 'BAIXO'
    ...
    if overdue_novos > 0:
        insights.append({
            'title': f'{overdue_novos} Novo epics com porte estÃ£o ATRASADOS',
            'text': f'Dos epics Novo com porte definido, {overdue_novos} jÃ¡ ultrapassaram o prazo WMI.',
            'level': 'danger'
        })
    ...
    if high_risk_techs:
        insights.append({
            'title': f'{len(high_risk_techs)} tÃ©cnico(s) acima da capacidade trimestral',
            'text': f'{", ".join(high_risk_techs)} tÃªm ocupaÃ§Ã£o >100%.',
            'level': 'danger'
        })
    ...
    if parallel_risk:
        insights.append({
            ...
            'text': 'Tocar 3 Novos simultÃ¢neos com 2-4h/dia cada pode pressionar a agenda.',
            'level': 'warning'
        })
    ...
    if available_techs:
        insights.append({
            'title': f'Capacidade disponÃ­vel: {", ".join(available_techs)}',
            'text': f'Estes tÃ©cnicos tÃªm espaÃ§o para absorver mais Novos.',
            'level': 'success'
        })
...
def generate_mock_data() -> Dict:
    ...
    mock_backlog = {
        ...
        'capacityTable': [
            ...
            {'name': 'Jorge', 'epicsAbertos': '8 (3N + 5U)', 'horasNovo': 326.0, 'horasUpsell': 22.0, 'totalRestante': 348.0, 'meses': 2.5, 'novosSimultaneos': '3 em andamento', 'ocupacao': 83.0, 'risco': 'MÃDIO'},
        ],
        'backlogNovo': [
            {'key': 'IWN-826', 'summary': 'DRA TÃNIA', 'assignee': 'Nino', 'porte': 'Large', 'status': 'Em andamento', 'gasto': 483.9, 'meta': 400.0, 'restante': 40.0, 'progresso': 1.21, 'criacao': '2025-10-10', 'prazoWmi': '2026-02-07', 'statusPrazo': '+72 dias', 'statusPrazoType': 'overdue'},
        ],
        'filaYasmin': [
        ...
        'backlogInsights': [
            {'title': '8 Novo epics com porte estÃ£o ATRASADOS', 'text': 'Dos epics Novo com porte definido, 8 jÃ¡ ultrapassaram o prazo WMI.', 'level': 'danger'},
            {'title': 'Anderson e Luiz Neto: 3.9 meses de backlog cada', 'text': 'Acima da capacidade trimestral (130% e 128%).', 'level': 'danger'},
            {'title': 'Paralelismo no limite', 'text': 'Luiz, Jorge e Nino tocam 3 Novos simultÃ¢neos.', 'level': 'warning'},
            {'title': 'Capacidade disponÃ­vel', 'text': 'Daniel e Fabio tÃªm espaÃ§o para absorver mais Novos.', 'level': 'success'},
        ]
    }
    ...
    return {
        ...
        'migracaoCloud': [{'key': 'IWN-3779', 'summary': 'Cloud Migration',
                           'assignee': 'Anderson', 'status': 'ConcluÃ­do', 'hours': 47.0}],
